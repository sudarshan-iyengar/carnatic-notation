...
import { isCellBlock, type NotationBlock, type NotationDocument, type NotationMetadata } from '../types/notation';
...
export const EditorWorkspace = ({ document, onUpdateMetadata, onUpdateBlock, onRemoveBlock }: EditorWorkspaceProps) => (
      ...
      <div className="w-full flex flex-col items-center mt-4">
        {document.blocks.map((block) => {
          if (block.type === 'heading') {
            return <HeadingBlock key={block.id} block={block} onUpdate={onUpdateBlock} onRemove={onRemoveBlock} />;
          }

          if (isCellBlock(block)) {
            return <AvartanamBlock key={block.id} block={block} onUpdate={onUpdateBlock} onRemove={onRemoveBlock} />;
          }
